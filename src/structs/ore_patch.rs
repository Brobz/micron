...
impl OrePatch {
    pub fn new(ore_type: OreType, density: u32, richness: u32) -> Self {
        Self {
            ore_type,
            density,
            richness,
    ...
    pub fn tick(&mut self, ent: &mut Ent, world_info: &mut WorldInfo) -> Option<GameObject> {
        ...
        if health_left != 1.0 && (health_left * 100.0) as u32 % self.density == 0 {
            // TODO: When  this block triggers, spawn Ore gameobject
            // units collect ore game objects automatically, but slowly with their ray, up to capacity
            // then they are slower, clunkier, and stop auto collecting
            // drop all on death on a big ore ball
            // ore ball radius represents value
            // when auto colelcting, ball slowly shrinks
            // if value = radius = 0, clear it from world
    ...
    fn drop_new_ore(&self, ent: &Ent, world_info: &mut WorldInfo) -> GameObject {
        let new_ent = Ent::new(
            ...
            ent.owner,
            self.richness,
            ent.position - Vector2D::<f32>::new(200.0, 200.0),
            Point::new(self.richness as i32, self.richness as i32),
            BLUE_RGB,
        );
